...
import { useImagesUpload } from '~/composables/account/useImagesUpload'
...
async function handleUpload() {
  ...
  errorMessage.value = null
  const res = await uploadImages(selectedFiles.value)
  if (!res) {
    ...
    <div class="space-y-5">
      ...
      <div v-if="previews.length" class="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[50vh] overflow-y-auto">
        <div v-for="(p, i) in previews" :key="p.url">
          ...
          <UButton color="error" variant="outline" class="mt-2" :disabled="isUploading" @click="removeFile(i)">
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <UButton :loading="isUploading" :disabled="!selectedFiles.length || isUploading" @click="handleUpload">
          Upload
        </UButton>
      </div>
    </div>
