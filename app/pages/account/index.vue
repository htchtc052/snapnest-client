...
import { useRenameImageFeature } from '~/composables/features/useRenameImageFeature'
import { useShareImagesFeature } from '~/composables/features/useShareImagesFeature'
import { useTrashImagesFeature } from '~/composables/features/useTrashImagesFeature'
...
const { renameImage } = useRenameImageFeature()
const { shareImages, isSharing } = useShareImagesFeature()
const { isTrashingImages, trashImages } = useTrashImagesFeature()
...
async function renameSelectedImage() {
  const updatedImage = await renameImage(selectedIds.value[0]!)
      ...
      <SelectionBar v-if="isSelectionMode" @clear="clearSelection">
        <div class="flex min-w-0 flex-1 items-center justify-between gap-3">
          ...
          <div class="flex shrink-0 items-center gap-1">
            <UButton icon="i-heroicons-share-20-solid" color="neutral" variant="ghost" size="sm" square title="Share"
              :loading="isSharing" @click="shareSelectedImages">
              <span class="hidden sm:inline">Share</span>
            </UButton>

            <UButton v-if="selectedIds.length === 1" icon="i-heroicons-pencil-square-20-solid" color="neutral"
              variant="ghost" size="sm" square title="Rename" @click="renameSelectedImage">
              <span class="hidden sm:inline">Rename</span>
            </UButton>

            <UButton icon="i-heroicons-archive-box-arrow-down-20-solid" color="neutral" variant="ghost" size="sm" square
              title="Move to trash" :loading="isTrashingImages" @click="trashSelectedImages">
